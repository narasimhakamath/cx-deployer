...
export default function DeploymentManager() {
  ...
  const handleLogout = async () => {
    try {
      await fetch("/cxdeployer/api/auth/logout", {
        method: "POST",
        credentials: "include",
      });
      router.push("/login");
  ...
  const fetchCurrentDeployment = async () => {
    try {
      const response = await fetch("/cxdeployer/api/current-deployment", {
        credentials: "include",
      });
  ...
  const pullLatestCode = async () => {
    setLoading(true);
    try {
      const response = await fetch("/cxdeployer/api/pull-code", {
        method: "POST",
        credentials: "include",
      });
  ...
  const deployCommit = async (commit) => {
    ...
    try {
      const response = await fetch("/cxdeployer/api/deploy", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: `Deploying commit ${commit.hash.substring(
            0,
            7
          )} from ${selectedBranch}`,
        });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage your DFL CX deployments
          </p>
        </div>
        ...
        <CardContent>
          {hasDeployment && currentDeployment ? (
              ...
              <div className="space-y-2">
                ...
                {currentDeployment.deployedBy && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {currentDeployment.deployedBy.name} (
                      {currentDeployment.deployedBy.email})
                    </span>
                  </div>
                )}
                <div className="flex items-center gap-2">
            ...
            <Alert>
              ...
              <AlertDescription>
                No deployments have been made yet. Select a branch and commit
                below to create your first deployment.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
