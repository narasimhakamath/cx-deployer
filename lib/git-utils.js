...
//     console.log("BRACNHESSSSSSSSSSSSSSSSSSSSSS------------------->", branches);
//     return branches;
//   } catch (error) {
...
export async function getBranches() {
  try {
    // Get remote branches sorted by most recent commit date
    const { stdout } = await execAsync(
      `git for-each-ref --sort=-committerdate --format="%(refname:short)" refs/remotes/${GIT_REMOTE_NAME}`,
      { cwd: GIT_REPO_PATH }
    );
      ...
      branchNames.map(async (branch) => {
        try {
          const { stdout: commitCount } = await execAsync(
            `git rev-list --count ${GIT_REMOTE_NAME}/${branch}`,
            { cwd: GIT_REPO_PATH }
          );

          const count = parseInt(commitCount.trim()) || 0;
          ...
          console.log(`${branch}: Error getting commit count`);
          return {
            name: branch,
            commitCount: 0,
          };
  ...
  try {
    ...
    try {
      const { stdout: trackingBranch } = await execAsync(
        `git rev-parse --abbrev-ref ${currentBranchName}@{upstream}`,
        {
          cwd: GIT_REPO_PATH,
        }
      );
      ...
      if (trackingBranch.trim()) {
        console.log(
          `Pulling latest changes for current branch: ${currentBranchName}`
        );
        const { stdout: pullOutput } = await execAsync(
          `git pull ${GIT_REMOTE_NAME} ${currentBranchName}`,
          {
            cwd: GIT_REPO_PATH,
          }
        );
    ...
    await execAsync(`git remote update ${GIT_REMOTE_NAME} --prune`, {
      cwd: GIT_REPO_PATH,
    });
    ...
    const { stdout: branchCount } = await execAsync(`git branch -r | wc -l`, {
      cwd: GIT_REPO_PATH,
    });
    const { stdout: commitCount } = await execAsync(
...
export async function getRepositoryStatus() {
  try {
    ...
    const { stdout: lastFetch } = await execAsync(
      `stat -c %Y .git/FETCH_HEAD 2>/dev/null || echo 0`,
      {
        cwd: GIT_REPO_PATH,
      }
    );
    ...
    return {
      ...
      lastFetchTime: new Date(
        Number.parseInt(lastFetch.trim()) * 1000
      ).toISOString(),
...
export async function getCurrentCommit() {
  try {
    const { stdout } = await execAsync(
      `git log -1 --format="%H|%s|%an|%ae|%ad" --date=iso`,
      { cwd: GIT_REPO_PATH }
    );
