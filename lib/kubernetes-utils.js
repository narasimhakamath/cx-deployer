...
export async function deployToKubernetes(imageTag, branch, commitHash, deploymentId) {
  try {
    ...
    const applyCommand = `kubectl -n ${NAMESPACE} apply -f fl-cx-cm.yaml`
    ...
    try {
      const configMapCommand = `kubectl -n ${NAMESPACE} apply -f fl-cx.yaml`
    ...
    const rolloutCommand = `kubectl -n ${NAMESPACE} rollout status deployment/fl-cx --timeout=300s`
