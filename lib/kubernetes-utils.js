...
const CONFIG_MAP_NAME = process.env.CONFIG_MAP_NAME
...
export async function deployToKubernetes(imageTag, branch, commitHash, deploymentId) {
  try {
    ...
    console.log(`Updating deployment YAML file: fl-cx-cm.yaml`)
    ...
    console.log(`Updating ConfigMap: fl-cx.yaml`)
    ...
    console.log(`Applying deployment: fl-cx-cm.yaml`)
    const applyCommand = KUBECTL_CONTEXT
      ? `kubectl --context=${KUBECTL_CONTEXT} -n ${NAMESPACE} apply -f fl-cx-cm.yaml`
      : `kubectl -n ${NAMESPACE} apply -f fl-cx-cm.yaml`
    ...
    try {
      const configMapCommand = KUBECTL_CONTEXT
        ? `kubectl --context=${KUBECTL_CONTEXT} -n ${NAMESPACE} apply -f fl-cx.yaml`
        : `kubectl -n ${NAMESPACE} apply -f fl-cx.yaml`
    ...
    const rolloutCommand = KUBECTL_CONTEXT
      ? `kubectl --context=${KUBECTL_CONTEXT} -n ${NAMESPACE} rollout status deployment/fl-cx --timeout=300s`
      : `kubectl -n ${NAMESPACE} rollout status deployment/fl-cx --timeout=300s`
...
async function updateDeploymentYaml(imageTag, deploymentId, commitHash, branch) {
  try {
    ...
    const deploymentPath = path.join(GIT_REPO_PATH, "fl-cx-cm.yaml")
...
async function updateConfigMap(commitHash, branch, deploymentId) {
  try {
    ...
    const configMapPath = path.join(GIT_REPO_PATH, "fl-cx.yaml")

    // Check if ConfigMap file exists
    try {
      await fs.access(configMapPath)
    } catch (accessError) {
      console.log("ConfigMap file fl-cx.yaml not found, skipping update")
      return false
    }
...
export async function getDeploymentStatus() {
  try {
    const command = KUBECTL_CONTEXT
      ? `kubectl --context=${KUBECTL_CONTEXT} -n ${NAMESPACE} get deployment fl-cx -o json`
      : `kubectl -n ${NAMESPACE} get deployment fl-cx -o json`
...
export async function getPodLogs(lines = 100) {
  try {
    const command = KUBECTL_CONTEXT
      ? `kubectl --context=${KUBECTL_CONTEXT} -n ${NAMESPACE} logs deployment/fl-cx --tail=${lines}`
      : `kubectl -n ${NAMESPACE} logs deployment/fl-cx --tail=${lines}`
